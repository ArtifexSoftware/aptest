...
def do_cibw(state):
    ...
    # 2026-09-16: We build all wheels first without testing:
    #
    # * Run cibw on each packge with CIBW_TEST_COMMAND unset.
    # * Run cibw on each packge with CIBW_TEST_COMMAND set.
    #
    # This is required so that we cope with mutually-dependent wheels such as
    # pymupdf4llm and pymupdf_layout.
    #
    for do_test in 0, 1:
        ...
        for command in state.commands:
            ...
            with pipcl.LogPrefix(f'{command}: '):
                ...
                if 0:
                    ...
                    def do_docutils(path):
                        path_out = f'{path}.html'
                        ...
                        if 1:
                            ...
                            html = pipcl.fs_read(path_out)
                            html = html.replace('<li><p>', '<li>')
                            html = html.replace('</p></li>', '</li>')
                            pipcl.fs_write(path_out, html)
                        pipcl.log(f'Have created: {path_out}')
                    do_docutils(f'{g_root}/README.rst')
