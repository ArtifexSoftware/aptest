...
def enter(*,
    ...
    if AUTOVENV_N < AUTOVENV_N_CURRENT:
        ...
        log(f'autovenv.enter(): Have entered autovenv venv {AUTOVENV_VENV_PATH!r}.')
        ...
        def setup(venv_path, packages, create):
            ...
            if create == 1 and os.path.isdir(venv_path):
                ...
                log(f'autovenv.enter(): {venv_path=}: Create/update venv with venv.create(), {kwargs=}.')
            ...
            log(f'autovenv.enter(): {venv_path=}: Updating environ to enter venv.')
            ...
            if do_setup_venv:
                ...
                if setupfn:
                    log(f'Calling venv setupfn().')
                    setupfn(builder_context)
                if packages:
                    if isinstance(packages, str):
                        packages = [packages]
                    log(f'autovenv.enter(): {venv_path=}: Installing packages: {packages}.')
                    subprocess.run(
                    ...
                    log(f'autovenv.enter(): {venv_path=}: No packages to install.')
